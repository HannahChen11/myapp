...
namespace 最佳工作序列
{
    class Program
    {
        ...
        static List<double> valueAverage = new List<double>();//每个工作每小时的平均价值
        static List<int> order = new List<int>();//最佳序列
        static string[] readText;


       
        static int Search(int[] Limit, int value)//找到期限的最小值并返回ID
        {
            int i = -1;
        ...
        {
            StreamReader sw = new StreamReader(@"F:\dataoftest.txt");
            ...
            readText = File.ReadAllLines(@"F:\dataoftest.txt");
        ...
        static void Main(string[] args)
        {
            ...
            while (order.Count < Limit.Count)//最佳序列的总数小于期限的数目
            {
               
                int i = Search(limit, limit.Min());//i是期限中的最小值
